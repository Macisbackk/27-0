...
export function ManagerMobilePlayBar() {
  ...
  return (
    <StickyActionBar aboveNav portal className="manager-mobile-play-bar ui-layer-mobile-action-bar">
      <div className="flex w-full flex-col gap-2">
        {lastAdvanceError ? (
          <div className="flex items-center justify-between gap-2 rounded-xl border border-amber-500/40 bg-amber-500/15 px-2.5 py-1.5 text-[11px] text-amber-100">
            <p className="min-w-0 flex-1 truncate leading-snug">{lastAdvanceError}</p>
            <button
              type="button"
              onClick={() => {
                autoPickSquad();
