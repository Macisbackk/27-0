...
export function simulateCareerToGameWeek(
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    // Sim-to-date must never accept a sale for the user.  Decline incoming
    // bids deterministically; informational inbox messages remain queued.
    if (hasBlockingManagerDecision(next)) {
      const bids = getPendingIncomingClubBids(next);
      if (bids.length > 0) {
        next = bids.reduce(
          (current, bid) => rejectIncomingOffer(current, bid.id),
          next
        );
        continue;
      }
    ...
    for (let friendlyStep = 0; friendlyStep < 8; friendlyStep++) {
      ...
      // A malformed/empty choice list is regenerated by autoSelectFriendlyForSim.
      // Keep trying rather than treating an unchosen friendly as a terminal state.
    }
